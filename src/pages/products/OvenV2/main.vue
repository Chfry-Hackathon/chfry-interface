<template>
  <div class="page-baskets">
    <div class="chr-title">
      ...
      <div class="t2">
        Stake and Grill your fUSD and convert them to <br />{{
          replaceSymbol("USDT/USDC/DAI over time")
        }}
      </div>
    </div>
    ...
    <div v-else>
      <div>
        <div class="item-con" v-if="loadInfo">
          ...
          <div class="btns" v-if="isMobile">
            <a-button
              class="chr-btn chr-btn22"
              @click="grill"
              :loading="grill_data.loading"
              :disabled="grill_data.disabled"
              >GRILL</a-button
            >
          </div>
          ...
          <div class="btns" v-if="isMobile">
            <a-button
              class="chr-btn chr-btn22"
              @click="withdraw"
              :loading="withdraw_data.loading"
              :disabled="withdraw_data.disabled"
              >WITHDRAW</a-button
            >
          </div>
        </div>
      ...
      >
        <token-msg
          v-for="item in curPool.totals.slice(0, 1)"
          :key="item.text"
          :token="item"
        ></token-msg>
        <token-msg
          v-for="item in curPool.totals.slice(4, 5)"
          :key="item.text"
          :token="item"
        ></token-msg>
      </div>
...
export default {
  ...
  methods: {
    ...
    async checkInfo(b) {
      ...
      let pool = {};
      if (b) {
        pool = this.pools[this.curSymbol] = (
          await getOvenPoolInfo(address,this.isOvenV2)
        ).data;
      } else {
        pool =
          this.pools[this.curSymbol] || (await getOvenPoolInfo(address,this.isOvenV2)).data;
        this.pools[this.curSymbol] = pool;
      }
      pool.address = address;
      let fryUSD = getTokenAddress("FRIES");
      pool.allowance = await getPoolAllowance(pool.address, fryUSD, "oven", this.isOvenV2);
