  ...
  <div class="oven-item">
    ...
    <div class="item">
      ...
      <div>
        ...
        <span class="icon">
          <img
            :src="require('../../../../assets/icon/' + data.symbol + '.svg')"
          />
        </span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    onChangePer(e) {
      ...
      this.amount = toFixed4(
        new BigNumber(this.data.amount1).times(this.curPer).times(0.01)
      );
  ...
  watch: {
    ...
    amount(a) {
      ...
      if (Number(this.amount) > 0) {
        if (Number(this.amount) > 0) {
          if (this.amount / this.data.amount1 < 0.001) {
            ...
            this.curPer = toFixed1(
              new BigNumber(this.amount)
                .times(100)
                .div(this.data.amount1)
                .toNumber()
            );
      ...
      window["ovenitem-amount-t"] = setTimeout(() => {
        //   },850);
...
@media screen and (max-width: 1279px) and (min-width: 751px) {
  .oven-item {
    ...
    background: url("../../../../assets/oven_box_bg1.svg#svgView(preserveAspectRatio(none))")
      no-repeat;
    -webkit-background-size: 100% 100%;
    background-size: 100% 100%;
